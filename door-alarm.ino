...
// --- Force-Styled Material 3 HTML/CSS ---
const char INDEX_HTML[] PROGMEM = R"rawliteral(
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Door Alarm System Panel</title>
...
<script>
  ...
  setInterval(U,500);window.onload=U;
</script></body></html>
)rawliteral";
      ...
      if (currentTaskTime - lastTelegramPoll >= 3000) {
        ...
        if (httpCode == HTTP_CODE_OK) {
          ...
          if (payload.indexOf(chatIdx) != -1) {
            ...
            else if (payload.indexOf("/hush") != -1) {
            ...
            else if (payload.indexOf("/testalarmnow") != -1) {
            ...
            else if (payload.indexOf("/rebootnow") != -1) {
...
void handleAction() {
  ...
  triggerUiFeedback(); 
  if (cmd == "arm") { if (isDoorOpen) { armPending = true; isArmed = false; sendAlert("Arming Pending"); } else { isArmed = true; armPending = false; sendAlert("System Armed Manually"); } } 
  else if (cmd == "disarm") { isArmed = false; armPending = false; isAlarmTriggered = false; isHushed = false; isTestingAlarm = false; nightDisarmTimer = millis(); nightTimerActive = true; sendAlert("System Disarmed Manually"); } 
  else if (cmd == "hush") { isTestingAlarm = false; if (isAlarmTriggered) { isHushed = true; sendAlert("Siren Hushed"); } if (!isDoorOpen) { isAlarmTriggered = false; isHushed = false; isArmed = true; } } 
  else if (cmd == "test") { isTestingAlarm = !isTestingAlarm; if (isTestingAlarm) sendAlert("Local Siren Testing Triggered"); else { isHushed = false; sendAlert("Local Siren Testing Terminated"); } } 
...
void setup() {
  ...
  server.on("/", handleRoot); server.on("/st", handleStatus); server.on("/ac", handleAction); server.begin();
}
...
void loop() {
  ...
  if (ntpInitialized && (now - lastTimeCheck > 5000)) { 
    ...
    if (getLocalTime(&timeinfo, 0)) { 
      ...
      if (!bootTimeSet) { if (isNight) { if (isDoorOpen) armPending = true; else isArmed = true; sendAlert("Night Mode Active: System Scheduled Auto-Arm Enabled"); } bootTimeSet = true; }
      if (isNight && !isArmed && !armPending && nightTimerActive) { if (now - nightDisarmTimer >= 3600000) { if (isDoorOpen) armPending = true; else isArmed = true; nightTimerActive = false; sendAlert("Night Mode Refresher: 1-Hour Disarm Bypass Concluded. Re-armed."); } } else if (!isNight) nightTimerActive = false; 
    }
  }
  bool currentDoorState = (digitalRead(REED_PIN) == HIGH);
  if (currentDoorState != lastDoorState) {
    isDoorOpen = currentDoorState;
    if (!isArmed && !isAlarmTriggered && !isTestingAlarm) { chimeTrigger = isDoorOpen ? 1 : 2; audioTimer = now; if (isDoorOpen) sendAlert("Door Opened"); else sendAlert("Door Closed"); }
    if (!isDoorOpen) { if (armPending) { isArmed = true; armPending = false; sendAlert("Status Update: Door Secured. System Completely Armed."); } if (isAlarmTriggered && isHushed) { isAlarmTriggered = false; isHushed = false; isArmed = true; sendAlert("Status Update: Door Secured post-hush. System Re-armed."); } }
    if (isDoorOpen && (isArmed || armPending)) { isAlarmTriggered = true; isHushed = false; isArmed = false; armPending = false; sendAlert("ALERT: Alarm Activated! Security zone breached."); }
    lastDoorState = currentDoorState;
  }
  if (isAlarmTriggered && isHushed) { setBuzzerVolume(0); } 
  else if ((isAlarmTriggered && !isHushed) || isTestingAlarm) { if ((now % 200) < 150) setBuzzerVolume(alarmVolume); else setBuzzerVolume(0); chimeTrigger = 0; uiBeepActive = false; } 
  else if (uiBeepActive) { if (now - uiBeepTimer < 60) setBuzzerVolume(chimeVolume); else { setBuzzerVolume(0); uiBeepActive = false; } }
  else if (chimeTrigger > 0) {
    unsigned long elapsed = now - audioTimer;
    if (chimeTrigger == 1) { if (elapsed < 120) setBuzzerVolume(chimeVolume); else if (elapsed < 160) setBuzzerVolume(0); else if (elapsed < 280) setBuzzerVolume(chimeVolume / 2); else { setBuzzerVolume(0); chimeTrigger = 0; } } 
    else if (chimeTrigger == 2) { if (elapsed < 120) setBuzzerVolume(chimeVolume); else if (elapsed < 180) setBuzzerVolume(0); else if (elapsed < 300) setBuzzerVolume(chimeVolume); else { setBuzzerVolume(0); chimeTrigger = 0; } }
  } else setBuzzerVolume(0); 
  setLedState(DOOR_LED_PIN, isDoorOpen ? AIRPLANE_BLINK : LED_ON);
  if (!wifiConnected) setLedState(ARMED_LED_PIN, RAPID_BLINK); else if (isAlarmTriggered) setLedState(ARMED_LED_PIN, SLOW_BLINK); else if (isArmed) setLedState(ARMED_LED_PIN, LED_ON); else setLedState(ARMED_LED_PIN, AIRPLANE_BLINK);
}
